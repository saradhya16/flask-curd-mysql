...
# xamp mysql
conn = mysql.connector.connect(user='root', password='', host='127.0.0.1', database='flask')
cur = conn.cursor()
app = Flask(__name__)
...
def insert():
 if request.method == "POST" or "GET":
  ...
  cur = conn.cursor()
  cur.execute('insert into student(name,age,location) values(%s,%s,%s)',(name,age,location));
  conn.commit()
  # cur.commit();
  cur.close()
...
def update(id):
 if request.method == 'POST':
  ...
  location = request.form['location']
  cur = conn.cursor()
  cur.execute("update student set name=%s,age=%s,location=%s  where id = %s",(name,age,location,id))
  conn.commit()
  cur.close()
